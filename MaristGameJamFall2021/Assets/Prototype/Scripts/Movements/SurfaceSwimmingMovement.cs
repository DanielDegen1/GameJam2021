...
public class SurfaceSwimmingMovement : MovementType
{
    ...
    public override void Movement()
    {
        ...
        Vector3 move = new Vector3(inputController.input.x, 0, inputController.input.y);
        ...
        if (inputController.elevate >= 0.02f && canJumpOutOfWater)
        {
            if (dif >= halfCrouch)
            {
        ...
        else if (inputController.elevate <= -0.02f)
            move.y = -1;
        else
        {
            float downWithOffset = cameraMovement.forward.y + 0.333f;
            float swimDown = Mathf.Clamp(downWithOffset * inputController.input.y, -Mathf.Infinity, 0f);
